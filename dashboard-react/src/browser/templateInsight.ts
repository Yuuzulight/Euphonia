// Ported from electron/src/templateInsight.ts -- same deterministic,
// zero-network insight logic, adapted to call dashboard-react's own
// zones.ts (whose zoneOf returns a Zone object, not just a name string).

import { PITCH_ZONES, F2_ZONES, HNR_ZONES, JITTER_ZONES, WEIGHT_ZONES, zoneOf, type Zone } from "../zones";
import type { Recording } from "../types";

function zoneName(zones: Zone[], v: number | null | undefined): string | null {
  return zoneOf(zones, v)?.name ?? null;
}
...
function buildSignals(r: Recording): Signal[] {
  const pitchZone = zoneName(PITCH_ZONES, r.pitch.mean_hz);
  const f2Zone = zoneName(F2_ZONES, r.formants.f2_hz);
  const hnrZone = zoneName(HNR_ZONES, r.voice_quality.hnr_db);
  const jitterZone = zoneName(JITTER_ZONES, r.voice_quality.jitter_pct);
  const weightZone = zoneName(WEIGHT_ZONES, r.weight?.h1a3c_db);
